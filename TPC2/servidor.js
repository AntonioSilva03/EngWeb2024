...
http.createServer((req, res) => {
    ...
    res.writeHead(200, { 'Content-Type': 'text/html; charset= utf-8' }); // Formatação das páginas em html

    // Página Inicial
    if (req.url == "/") {
        res.write("<div style='text-align: center;'><h1>Escola de Música</h1></div>"); // Alinhar texto no centro
    ...
    // Lista dos alunos
        ...
        axios.get("http://localhost:3000/alunos?_sort=nome").then((resp) => {
            ...
            res.write("</ul>");
            res.write("<h3><a href='/'> Voltar à pagina inicial </a> </h3>") // Botão de retorno à página inicial
            res.end();
        }).catch((erro) => {
    ...
    // Página de um aluno em específico
        ...
        axios.get("http://localhost:3000/alunos/" + alunoID).then((resp) => {
            ...
            res.write("<a href='/cursos/" + aluno.curso + "'>Curso: " + aluno.curso + "</a>"); // Hiperligação para a página do curso deste aluno
            res.write("<p>Ano: " + aluno.anoCurso + "</p>");
            res.write("<a href='/instrumentos/" + aluno.instrumento + "'>Instrumento: " + aluno.instrumento + "</a>"); // Hiperligação para a página do instrumento deste aluno
            res.write("<h3><a href='/alunos'> Voltar à lista de alunos </a> </h3>") // Botão de retorno à lista de alunos
            res.end();
    ...
    // Página de um curso em específico
        ...
        axios.get("http://localhost:3000/cursos/" + cursoID).then((resp) => {
            ...
            res.write("<a href='/instrumentos/" + curso.instrumento["#text"] + "'>Instrumento: " + curso.instrumento["#text"] + "</a>"); // Hiperligação para a página do instrumento deste curso
            res.write("<h3><a href='/cursos'> Voltar à lista de cursos </a> </h3>") // Botão de retorno à lista de cursos
            res.end();
    ...
    // Lista dos instrumentos
        ...
        axios.get("http://localhost:3000/instrumentos?_sort=%23text").then((resp) => {
            ...
            res.write("<ol>");
            data.forEach(i => { // Colocar cada instrumento na lista
    ...
    // Página de um instrumento em específico
        ...
        axios.get("http://localhost:3000/instrumentos?%23text=" + instrumentoNOME).then((resp) => {
            ...
            res.write("<div style='text-align: center;'><h1>Instrumento ID:" + instrumento.id + "</h1></div>");
            res.write("<h3>Instrumento: " + instrumento["#text"] + "</h3>");
            res.write("<h3><a href='/instrumentos'> Voltar à lista de instrumentos </a> </h3>") // Botão de retorno à lista de instrumentos
            res.end();
    ...
    //Tratamento de operação não suportada
    else { 
        res.write("<div style='text-align: center;'><h1> Operação não suportada </h1></div>");
        res.end();
    }

}).listen(3001) // Servidor fica à escuta na porta 3001
